    ...
    def add_new(self,wb,ws,ws1,ws2,inv):            
        def b1():
            ...
            if foodtype=='' or foodno=='' or foodname=='' or qty=='' or price=='' :
                tkMessageBox.showerror('Error','All fields are mandatory!')
            else:
                for i in range(2,maxrow+1):
                    if str(foodno).upper() == str(ws['b'+str(i)].value).upper():                  
                        tkMessageBox.showerror('Error','Food Number already Exists. \n Please choose a new one')
                        flag=False     
                if flag:
                    if foodtype.upper() == "CHINESE" or foodtype.upper() == "CONTINENTAL" or foodtype.upper() == "SOUTH" or foodtype.upper() == "NORTH":
                        try:
                            if int(qty)>0 and int(price)>=0:
                                ...
                                tkMessageBox.showinfo('Success','New Food added')
                                add.destroy()
                                inv.destroy()
                                self.inventory()
                            elif qty<=0:
                                tkMessageBox.showerror('Error','Quantity must be a positive non-zero integer')
                        except:
                            tkMessageBox.showerror('Error','Wrong Data for Quantity or Price')
                    else:
                            tkMessageBox.showerror('Error','Please enter a valid foodtype')
    ...
    def modify_food(self,wb,ws,ws1,ws2,inv,num1):
        def modify(somelist):
            ...
            if (foodtype.upper()==somelist[0]) and (foodno.upper()==somelist[1]) and (foodname  ==somelist[2]) and (qty==somelist[3]) and (price==somelist[4]):
                tkMessageBox.showinfo('Notification','No changes have been made')
                modify_main.destroy()
            else:
                if foodtype=='' or foodno=='' or foodname=='' or qty=='' or price=='':
                    tkMessageBox.showerror('Error','All fields are mandatory')
                        ...
                        if price=='' or str(price).isdigit():
                            if qty>=0 and price>=0:
                                ...
                                if foodtype=='' and foodno=='' and foodname=='' and qty=='' and price=='':
                                    ...
                                    tkMessageBox.showinfo('SUCCESS!','Food Item Modified')
                                    modify_main.destroy()
                                    inv.destroy()
                                    self.inventory()

                            else:
                                tkMessageBox.showerror('Error',"Negative values for field not allowed")
                        else:
                            tkMessageBox.showerror('Error',"Only positive numerical characters are allowed for price")
                    else:
                        tkMessageBox.showerror('Error',"Only positive numerical characters are allowed for quantity")
                else:
                    tkMessageBox.showerror('Error',"Enter a valid foodtype \n NORTH/SOUTH/CONTINENTAL/CHINESE")
        ...
        def delete_food():
            ...
            if foodno != '' or foodname != '' :
                for i in range(2,maxrow+1):
                    ...
                    tkMessageBox.showerror('Error','Food item does not exist. \n Enter a valid food item to delete')
            else:
                tkMessageBox.showerror('Error','Both fields are mandatory')
            if check==1:
                ...
                tkMessageBox.showinfo('Success','Food Item Deleted')
        ...
        def search1():
            ...
            if foodname != '':
                for i in range(2,maxrow+1):
                    ...
                    tkMessageBox.showinfo('Search Results','Food item does not exist')
                    sbn.destroy()
            else:
                tkMessageBox.showerror('Error','All fields are mandatory!')
    ...
    def generate_report(self,wb,ws,ws1,ws2):
        def clear_report():
            for j in range(2,row+1):
                ...
                wb.save('master_sheet.xlsx')
                wb.close()
            tkMessageBox.showinfo('The Food Box - Notification','Report has been reset for the day')
        ...
        def inc_and_dec(x,y,z,action):
            if action=='add':
                if x<int(self.ws_inv['d'+str(y)].value):
                   ...
                   tkMessageBox.showerror("Error","This is the maximum stock in the inventory",parent=sales2)                              
            elif action=='remove':
                if int(z['text']) >=1:
                    z['text']=str(x-1)
                    z.grid(row=y,column=10,sticky=W)
    ...
    def Checkout(self,sales2):
        ...
        for i in range(2,row-1):
            if self.dict1[i]==0:
                xz+=1
        if xz==(row-3):
            tkMessageBox.showerror('Error','No Food Ordered!\nPlease Order Something!!!')
        else:
            for i in range(2,row+1):
                string='d'+str(i)
                try:
    ...
    def validate():
        ...
        if x==1:
            ...
            for i in range(1,row1+1):
                ...
                tkMessageBox.showerror('Error','Wrong username or password',parent=root_login)
        elif x==0:
            ws1=wb['Admin']; row1=ws1.max_row
            ...
            if len(pass_1)>=8:
                ...
                if len(lst1)>0 and len(lst2)>0 and len(lst3)>0:
                    if (pass_1==pass_2) and person==0:
                        ...
                        tkMessageBox.showinfo('Success','Signup successful!',parent=root1)
                        root1.destroy()
                        ...
                        tkMessageBox.showerror('Error','Passwords don\'t match',parent=root1)
                else:
                    tkMessageBox.showerror('Error','Password must contain combination of\nletters,digits and special characters',parent=root1)
            else:
                tkMessageBox.showerror('Error','Password must be atleast 8 characters long',parent=root1)
        else:
            tkMessageBox.showerror('Error','No name or username given',parent=root1)
